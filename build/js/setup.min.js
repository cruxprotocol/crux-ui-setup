...
$(document).ready(function () {
  ...
  ********************************/
  ...
  function curlistAdapter(assetList, clientMapping) {
    ...
    var assetIdtoClientidMap = {};

    for (var clientKey in clientMapping) {
      assetIdtoClientidMap[clientMapping[clientKey]] = clientKey;
    }

    console.log("assetIdtoClientidMap is:- ", assetIdtoClientidMap);
    var applicableAssetIdList = [];

    for (var i in assetList) {
      var current = assetList[i];

      if (current.asset_id in assetIdtoClientidMap) {
        var currentAsset = {
          name: current.name,
          symbol: assetIdtoClientidMap[current.asset_id],
          img: current.image_sm_url
        };
        applicableAssetIdList.push(currentAsset);
      }
    }

    return applicableAssetIdList; // return assetList.map((e) => {
    //     return {
    //         name: e.name,
